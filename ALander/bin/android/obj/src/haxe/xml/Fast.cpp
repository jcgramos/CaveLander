...
HX_STACK_ARG(x,"x")
{
	...
	if (((bool((x->nodeType != ::Xml_obj::Document)) && bool((x->nodeType != ::Xml_obj::Element))))){
		HX_STACK_LINE(124)
		HX_STACK_DO_THROW((HX_CSTRING("Invalid nodeType ") + ::Std_obj::string(x->nodeType)));
	}
	HX_STACK_LINE(125)
	this->x = x;
	HX_STACK_LINE(126)
...
::String Fast_obj::get_innerData( ){
	...
	if ((!(it->__Field(HX_CSTRING("hasNext"),true)()))){
		HX_STACK_LINE(140)
		HX_STACK_DO_THROW((this->get_name() + HX_CSTRING(" does not have data")));
	}
	HX_STACK_LINE(141)
	...
	if (((n != null()))){
		HX_STACK_LINE(145)
		if (((  (((bool((v->nodeType == ::Xml_obj::PCData)) && bool((n->nodeType == ::Xml_obj::CData))))) ? bool((::StringTools_obj::trim(v->get_nodeValue()) == HX_CSTRING(""))) : bool(false) ))){
			HX_STACK_LINE(146)
			...
			struct _Function_3_1{
				inline static bool Block( Dynamic &it,::Xml &n2){
					...
					{
						HX_STACK_LINE(147)
						return (  (((  (((n2->nodeType == ::Xml_obj::PCData))) ? bool((::StringTools_obj::trim(n2->get_nodeValue()) == HX_CSTRING(""))) : bool(false) ))) ? bool((it->__Field(HX_CSTRING("next"),true)() == null())) : bool(false) );
					}
					return null();
				}
			};
		...
		HX_STACK_LINE(150)
		HX_STACK_DO_THROW((this->get_name() + HX_CSTRING(" does not only have data")));
	}
	HX_STACK_LINE(152)
